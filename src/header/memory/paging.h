...
struct PageDirectoryEntryFlag {
    ...
    uint8_t write_bit     : 1;  // bit 1
    uint8_t user_bit      : 1;  // bit 2
    uint8_t pwt           : 1;  // bit 3
    uint8_t pcd           : 1;  // bit 4
    uint8_t accessed      : 1;  // bit 5
    uint8_t dirty         : 1;  // bit 6
    uint8_t use_pagesize_4_mb  : 1;  // bit 7
...
struct PageDirectoryEntry {
    struct PageDirectoryEntryFlag flag;   // bits 7..0

    uint16_t global_page    : 1;  // bit 8
    uint16_t available      : 3;  // bits 11..9 (software)
    uint16_t pat            : 1;  // bit 12 (PAT for 4 MiB)
    uint16_t higher_address : 8;  // bits 20..13 (PSE-36 high PA); 0 if unused
    uint16_t reserved_2     : 1;  // bit 21 (must be 0)

    uint16_t lower_address  : 10; // bits 31..22
} __attribute__((packed));
