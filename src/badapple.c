...
#define COLOR_VIDEO      0x0F      /* white on black */
#define FRAME_COLS       80
#define FRAME_ROWS       25
#define FRAME_DELAY_MS   40        /* ~25 fps (1000 / 25) */
#define CTRL_C_CODE      3         /* keyboard.c maps Ctrl+C to 3 */

#define ANIM_BUF_SIZE    (1024u * 1024u)  /* 1 MiB */

#define ANIM_FILENAME    "anim.txt"
#define ANIM_NAME_LEN    8
...
static bool check_ctrl_c(void) {
    ...
    /* Drain keyboard buffer; stop if we see Ctrl+C */
    do {
        sys_getchar(&k);
        if (k == CTRL_C_CODE) {
...
/* Draw one frame from a 2D char array onto text framebuffer */
...
/* Clear frame buffer in memory to spaces */
...
int badapple_play(void) {
    ...
    if (rc != 0) {
        const char *msg = "badapple: failed to read /anim.txt (ext2 error)\n";
